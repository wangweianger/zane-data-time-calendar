...
var calendar = function () {
	...
	_createClass(calendar, [{
		...
		value: function getTimeDates(deraultDay) {
			...
			for (var _i = 0, _len = monTotalDay; _i < _len; _i++) {
				var _weekday = (firstDayMeek + _i) % 7;
				timeDatas.push({
					day: _i + 1,
					week: _weekday,
					daytype: 'now',
					fullday: year + '/' + month + '/' + (_i + 1)
				});
			}
			if (lastDayMeek < 6) {
				var _preyear = year;
				...
				for (var _i2 = 0, _len2 = 6 - lastDayMeek; _i2 < _len2; _i2++) {
					var _weekday2 = (lastDayMeek + 1 + _i2) % 7;
					timeDatas.push({
						day: _i2 + 1,
						week: _weekday2,
						class: 'light',
						daytype: 'next',
						fullday: _preyear + '/' + _premonth + '/' + (_i2 + 1)
					});
				}
			}
			return {
		...
		value: function preMonth(year, month) {
			...
			var tnodyhtml = this.mainCheckDayHTML(json);
		...
		value: function getYearHtml(year) {
			...
			var tnodyhtml = this.mainCheckYearHTML(yearDatas);
		...
		value: function monthHTML(monthDatas) {
			var tnodyhtml = this.mainCheckMonthHTML(monthDatas);
